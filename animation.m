...
%prefix = '~/School/Grade 5/TFAE01 - Examensarbete för civilingenjörsexamen i teknisk fysik/FortranPractice/Recreation/';
prefix = '~/Dropbox/School/LAquila/Paper1/more solvent/';
...
while go
    ...
    % Reset the lower limit for each iteration.
    if sequence == true
        lowLim = 1;
    else
        lowLim = b;
        ...
        if criticalRegion == true && x0(n) > 0
            ...
            if cellVisualisation == true
                if linInt == true
                    ...
                    critRows = critRows(critRows <= size(imInt,2));
                else
                    critRows = [temp];
                end
            else
...
%     critRows(1)
    ...
    if export == true && sum(f == 'pdf') == 3
        h = rectangle('Position',[0 (floor(x0(n))-1/2)*scalefactor length+1 scalefactor], 'FaceColor', 'b'); h.FaceColor(4)=0.305; h.EdgeColor(4)=0.0;
    else
        h = rectangle('Position',[0 (floor(x0(n))-1/2)*scalefactor length+1 scalefactor], 'FaceColor', 'b'); h.FaceColor(4)=0.3; h.EdgeColor(4)=0.0;
    end
%     if export == true && sum(f == 'pdf') == 3
