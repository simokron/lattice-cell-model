...
cutoff = 15; % The number of data points to exclude near the end.

%cutoff = [10 20 30 30 20]; % L = 512; 0.40
...
%cutoff = [26 60 60 60 60]; % L = 512; 0.25

%cutoff = [25 10 10 10 10]; % L = 1024; 0.40
%cutoff = [35 20 20 15 15]; % L = 1024; 0.35
%cutoff = [50 30 30 25 20]; % L = 1024; 0.30
%cutoff = [60 35 35 30 25]; % L = 1024; 0.25
...
export = true; f = 'png'; %Turns on the frame export of type 'f' - supports pdf or png!
...
function generatePlot(L, lambda, x0, MCS, cutoff, logLog, fontSize, allRuns, k, maxVal)
    ...
    if logLog == false
        ...
        if allRuns == false
            ...
            if k == maxVal
                %plot([0 2^10],0.5*[0 2^10]-0.42,'-m', 'DisplayName', 'Fickian: $y \propto x^{0.5}$') % L=512; 0.40
                %plot([0 2^10],0.5*[0 2^10]-0.57,'-m', 'DisplayName', 'Fickian: $y \propto x^{0.5}$') % L=512; 0.35
                %plot([0 2^10],0.5*[0 2^10]-0.90,'-m', 'DisplayName', 'Fickian: $y \propto x^{0.5}$') % L=512; 0.25
                
                %plot([0 2^10],0.5*[0 2^10]-0.48,'-m', 'DisplayName', 'Fickian: $y \propto x^{0.5}$') % L=1024; 0.40
                %plot([0 2^10],0.5*[0 2^10]-0.57,'-m', 'DisplayName', 'Fickian: $y \propto x^{0.5}$') % L=1024; 0.35
                %plot([0 2^10],0.5*[0 2^10]-0.72,'-m', 'DisplayName', 'Fickian: $y \propto x^{0.5}$') % L=1024; 0.30
                plot([0 2^10],0.5*[0 2^10]-0.83,'-m', 'DisplayName', 'Fickian: $y \propto x^{0.5}$') % L=1024; 0.25
            end
        end
