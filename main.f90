            ...
            elseif(topView .eqv. .true.) then
                if(noEvap .eqv. .false.) then
                    if(spin == 0) then
                        ...
                        if(P < pEvap) call evap(j_s, i_s, j_c, i_c, sigma, numSpins)
                        GO TO 10
                    elseif(spin_p == 0) then
                        call random_number(P) !Compare to a pseudo-random number between 0 and 1.
                        if(P < pEvap) call evap(j_s_p, i_s_p, j_c_p, i_c_p, sigma, numSpins)
                        GO TO 10
                    endif
                endif
            endif
...
program main
    ...
    character(256) :: file_id, file_name, folderName
    character :: d, d2
