...
FourierTransform = true; stretchCS = false; FTMap = jet(2^cD); %disables gridOn an shows the fft2 image; stretchCS applies a stretched colour-space of type FTMap;
...
[lambda, L, numIters] = findParamaters(directory);

% Runs the preChecks function which does some initial debugging.
[once, x0, rawMap, lowLim, go, tempPause, k] = preChecks(directory, f, sequence, export, criticalRegion);
...
function [lambda, L, numIters] = findParamaters(temp)
    fprintf('Extracting the paramaters from the directory name...\n')

    ipos = strfind(temp,'lambda_') + strlength("lambda_");
    iposLim = strfind(temp,'-L_') - 1;
    lambda = str2num(temp(ipos:iposLim));

    ipos = strfind(temp,'L_') + strlength("L_");
    iposLim = strfind(temp,'-J_') - 1;
    L = str2num(temp(ipos:iposLim));

    ipos = strfind(temp,'numIters_2-') + strlength("numIters_2-");
    iposLim = strfind(temp,'-initialDist_') - 1;
    exponent = str2num(temp(ipos:iposLim));
    numIters = 2^exponent;

    fprintf(['Done! Found lambda = ' num2str(lambda) ', L = ' num2str(L) ' and numIters = 2^' num2str(log2(numIters)) '.\n'])
end

% This function does some initial debugging to catcha ny obvious mistakes, such as the directory being empty.
function [once, x0, rawMap, lowLim, go, tempPause, k] = preChecks(tempDir, f, sequence, export, criticalRegion)
    a = dir([tempDir '/*.dat']);
    b = numel(a);
    if b == 0
    ...
    if criticalRegion == true
        fprintf('\nLoading the j_demix vector. Please double-check that this is the correct vector\n(rerun solventDistribution with saveMat = true if you''re uncertain).\n')
        load('x0.mat','x0Exp');
        x0 = x0Exp;
    else
        x0 = [];
...
function setDimensionsPeaks
    global lambda L fontSize c0 MCS f export n frameUpdated
...
function exportFrame
    global export c0 MCS f directory pauseTime n b
